...
class Parser {
    fun parseAssFile(inputStream: InputStream): Ass {
...
class ParserIS {
    ...
    fun createAss(filename: String, ass: Ass, characters: List<String>): StreamResource? {
        val lines: MutableList<String> = mutableListOf()
        if (ass.events.dialogues.any { it.actor in characters }) {
            ...
            ass.events.dialogues.forEach { dialogue ->
                val chars = dialogue.actor.split("and", ",").map { it.trim() }
